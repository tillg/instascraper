...
- [ ] Add `instascraper/behavior.py` with `Range` (`lo`, `hi`, `sample(rng)`)
      and the frozen `BehaviorProfile` dataclass (all fields + defaults from
      `architecture.md` §1).
- [ ] Add `Humanizer(profile, rng=None, sleep=time.sleep, now=time.monotonic,
      ...
      `warmup_calls`).
- [ ] **Test**: seeded RNG → `delay(kind)` returns values within range for every
...
- [ ] Add `Humanizer.should_stop_early()` → `rng` vs `early_stop_prob`.
- [ ] **Test**: `early_stop_prob=0` never stops; `=1` always stops; intermediate
      value stops at the expected rate over N calls with a seeded RNG.

## 3. Rate ceilings & active-hours gating

- [ ] Add request-timestamp `deque` + session counters; implement `gate(kind)`
...
- [ ] Add `record()` to log a request/post against the counters + window.
- [ ] **Test**: fake `now()`/`wall` → window ceiling yields a **bounded** `WAIT`;
...
- [ ] Add `Humanizer.backoff(attempt)` → jittered `backoff_base * 2**attempt`
      capped at `backoff_max`; helper to know when `backoff_attempts` is
      exhausted.
- [ ] **Test**: fake sleep records the schedule; values respect the cap and
      jitter bounds; gives up after `backoff_attempts`.

## 5. Config parsing & profile builder

- [ ] Add `build_profile(opts)` parsing `"lo,hi"` range strings and scalars into
      a `BehaviorProfile`; `enabled=False` when humanization is off.
- [ ] Add `INSTASCRAPE_HUMANIZE_*` entries to `config.ENV_KEYS`.
- [ ] **Test**: `"2,7"` → `Range(2.0, 7.0)`; malformed strings raise a clear
      error; round-trip through save/load config; `--no-humanize` → `enabled
      False`.

## 6. Wire into auth

- [ ] In `auth.get_client`, set `client.delay_range` from
...
- [ ] **Test**: `delay_range` reflects the profile; `warmup_calls=0` makes no
      extra calls; existing auth-helper tests still pass.

## 6b. Device-identity continuity (see `architecture.md` §1b; field-evidenced)

- [ ] Add a `device_profile` option (`ios` | `android`, default `ios` to match
      this account's real usage) resolved via the same precedence chain; seed
...
- [ ] **Migration — never re-fingerprint a live session** (`architecture.md`
...
- [ ] Ensure no code path adds a speculative re-login; a fresh `Client.login` is
      a flag-risk event (a fresh Chrome login already tripped a new-device alert).
- [ ] **Test**: a **new** session gets the configured device family and a stable
...
- [ ] Ensure `humanizer=None` reproduces today's exhaustive paging exactly (the
      library path).
- [ ] **Test**: with a seeded humanizer, paging stops early at the expected rate;
...
- [ ] Build the profile from resolved options; construct one `Humanizer`; pass it
      to `get_client` and `scrape`.
- [ ] Replace the fixed inter-post `--delay` sleep with `humanizer.delay("post")`;
      call `gate("post")` before each post (`WAIT` → sleep+recheck, `STOP` →
      graceful end with `EXIT_PARTIAL` and a clear message).
- [ ] On `PleaseWaitFewMinutes`, use `humanizer.backoff()` retry loop instead of
      immediate `EXIT_FATAL`; keep `LoginRequired`/`ChallengeRequired` fatal.
- [ ] Add CLI flags for every parameter + `--no-humanize`. **Do not alias
      ...
      surprise on upgrade).
- [ ] Update the `cli.py:245` progress banner ("paced ~1–3s/request, Ns between
      posts") so it no longer hardcodes the old pacing under humanization.
- [ ] **Test**: `resolve_options` + `build_profile` precedence (CLI > .env > env >
...
- [ ] Record the effective humanization summary (enabled + key ranges, or a short
...
- [ ] **Test**: `render_metadata`/`render_markdown` include the humanization
      summary and `comments_scanned`; when early-stop fires, `comments_scanned <
      comment_scan_limit`; provenance tests updated.

## 10. Docs

- [ ] Update `README.md`: new humanization flags, defaults, `--no-humanize`, and
      a short "why this exists" note (behavioral realism ≠ guarantee; ToS caveat
      unchanged).
- [ ] Update `specs/system/architecture.md` and `specs/system/domain.md` to add
...
- [ ] Run the whole suite (`pytest`); confirm it is green, network-free, and does
      not actually sleep.
